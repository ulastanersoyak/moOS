...
static int32_t
is_file_path_valid (const char *path)
{
  size_t size = strlen (path);
  // TODO: replace memcmp with strncmp
  if (size > 3 && size < MAX_PATH_LEN && isdigit (path[0])
      && memcmp (path, ":/", 2))
    {
...
static void
get_body (const char **path, struct path_body *body)
{
...
void
write_path (const struct path_root *root)
{
...
void
free_path_heap (struct path_root *root)
{
  struct path_body *body = root->body;
  while (body)
    {
