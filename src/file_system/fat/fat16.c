...
static void fat16_init_private(struct disk_t *disk,
...
static int32_t fat16_total_items_in_dir(struct disk_t *disk,
                                        uint32_t dir_start_sec) {
  struct FAT_dir_item item;
...
static int32_t fat16_get_root_dir(struct disk_t *disk,
                                  struct FAT_private *private,
  ...
  total_sectors =
      (root_dir_size % disk->sector_size) ? total_sectors + 1 : total_sectors;
...
static void get_relative_file_name(struct FAT_dir_item *item, char *buffer,
                                   size_t size) {
...
static struct FAT_dir_item *clone_dir_item(struct FAT_dir_item *item,
                                           size_t size) {
...
static int32_t get_cluster_for_offset(struct disk_t *disk, int32_t start,
                                      int32_t offset, int32_t size_of_cluster) {
  // struct FAT_private *priv = disk->fs_private_data;
...
static int32_t fat16_read_internal(struct disk_t *disk, int32_t cluster,
...
static struct FAT_directory *load_fat_dir(struct disk_t *disk,
                                          struct FAT_dir_item *item) {
  struct FAT_private *priv = disk->fs_private_data;
...
static struct FAT_item *fat_item_init(struct disk_t *disk,
                                      struct FAT_dir_item *item) {
...
static struct FAT_item *find_item_in_dir(struct disk_t *disk,
                                         struct FAT_directory *dir,
...
static struct FAT_item *get_dir_entry(struct disk_t *disk,
...
void *fat16_open(struct disk_t *disk, struct path_root *root,
                 enum FILE_MODE mode) {
