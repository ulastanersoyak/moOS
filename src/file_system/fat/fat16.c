...
static void format_file_name(char **buffer, const char *input){
...
static void get_relative_file_name(struct FAT_dir_item* item, char *buffer, size_t size){
...
static struct FAT_dir_item *clone_dir_item(struct FAT_dir_item *item, size_t size){
...
static int32_t get_fat_entry(struct disk_t *disk, int32_t cluster_to_use){
  struct FAT_private *priv = disk->fs_private_data;
  struct disk_stream *stream = priv->FAT_read_stream;
  if(!stream){
    return -IO_ERROR;
  }
  uint32_t fat_pos = priv->header.primary_header.reserved_sectors * disk->sector_size;
...
static int32_t get_cluster_for_offset(struct disk_t *disk, int32_t start, int32_t offset, int32_t size_of_cluster){
  // struct FAT_private *priv = disk->fs_private_data;
...
static int32_t fat16_read_internal(struct disk_t *disk, int32_t cluster, int32_t offset, int32_t total, void *buffer){
  struct FAT_private *priv = disk->fs_private_data;
  struct disk_stream *stream = priv->cluster_read_stream;
  int32_t size_of_cluster = priv->header.primary_header.sectors_per_cluster * disk->sector_size;
  int32_t cluster_to_use = get_cluster_for_offset(disk, cluster, offset, size_of_cluster);
  if(cluster_to_use < 0){
    return cluster_to_use;
  }
  int32_t offset_from_cluster = offset % size_of_cluster;
  int32_t start_sector = priv->root_dir.sector_end_pos + ((cluster - 2) * priv->header.primary_header.sectors_per_cluster);
  ...
  total -= total_to_read;
  if(total > 0){
    fat16_read_internal(disk, cluster, offset + total_to_read, total, buffer + total_to_read); 
  }
  return 0;
} 
...
static struct FAT_directory *load_fat_dir(struct disk_t *disk, struct FAT_dir_item *item){
  struct FAT_private *priv = disk->fs_private_data;
  ...
  int32_t cluster = item->high_16_bit_first_cluster | item->low_16_bit_first_cluster;
  int32_t cluster_sector = priv->root_dir.sector_end_pos + ((cluster - 2) * priv->header.primary_header.sectors_per_cluster);
...
static struct FAT_item *fat_item_init(struct disk_t *disk, struct FAT_dir_item *item){
...
static struct FAT_item *find_item_in_dir(struct disk_t *disk, struct FAT_directory *dir, const char* path_part){
  ...
  for(size_t i = 0; i< dir->total; i++){
    get_relative_file_name(&dir->item[i], temp_file_name, sizeof(temp_file_name));
...
static struct FAT_item *get_dir_entry(struct disk_t *disk, struct path_body *body){
  struct FAT_private *priv = disk->fs_private_data;
  struct FAT_item *current_item = 0;
  //get the root item for given path
  struct FAT_item *root_item = find_item_in_dir(disk, &priv->root_dir, body->body_str);
  if(!root_item){
    return current_item;
  }
  ...
  while(next_part != 0){
    ...
    struct FAT_item *tmp_item = find_item_in_dir(disk, current_item->dir, next_part->body_str);
