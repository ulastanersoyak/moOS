...
int32_t disk_stream_read(struct disk_stream *stream, void *buffer, size_t len) {
  ...
  int32_t total =
      len > MASTER_MAIN_DISK_SECTOR_SIZE ? MASTER_MAIN_DISK_SECTOR_SIZE : len;
...
void stream_seek(struct disk_stream *stream, int32_t pos) { stream->pos = pos; }

void stream_free(struct disk_stream *stream) { free(stream); }
